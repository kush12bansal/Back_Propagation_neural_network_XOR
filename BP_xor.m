...
X = [0.1,0.1; 0.1,.9; .9,0.1; .9,.9]; % here 0.1 is for 0 and 0.9 is for 1 in normal xor logic table.
Y = [0.1; .9; .9; 0.1];

% Initialize weights and biases
W1 = randn(2,3); % weights for hidden layer
...
% Test the network
X=[0.1,.9]; % check with input from user
hidden_layer_input = X * W1 + b1;
hidden_layer_output = sigmoid(hidden_layer_input);
output_layer_input = hidden_layer_output * W2 + b2;
output_layer_output = sigmoid(output_layer_input);
